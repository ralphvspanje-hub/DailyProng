# Powerhouse System — Execution Guide
...
Open Claude Desktop. Switch to Cowork. Point it at the `powerhouse-system/` folder.
...
Open a new Cowork session. Point it at the same `powerhouse-system/` folder.
...
2. **`onboarding.md` + temporary `CLAUDE.md`** — the agent writes `onboarding.md` plus a temp `CLAUDE.md` with context-rich handoff language (e.g., "You are the Powerhouse learning system. The user needs onboarding. Read `onboarding.md` and begin the onboarding conversation.")
...
1. Open Cowork → point at `powerhouse-system/`
2. Type "go"
